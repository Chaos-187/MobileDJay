/**
 * Customer payment rules: deposit first, balance from N days before event start.
 */
...
function customerPaymentOptions(params) {
    ...
    const balanceWindowOpen = dueDate ? now.getTime() >= dueDate.getTime() : false;
    ...
    if (open && quoteTotal > 0 && balanceRemaining > 0.005) {
        ...
        } else if (!balanceDueAt) {
            balance_block_reason = 'no_event_date';
        } else if (!balanceWindowOpen) {
            balance_block_reason = 'not_yet_due';
        } else {
...
function customerBalanceBlockedMessage(options) {
    ...
    if (options.balance_block_reason === 'deposit_first') {
        return 'Pay your deposit first, then the remaining balance can be paid closer to your event date.';
    }
    if (options.balance_block_reason === 'not_yet_due' && dueLabel) {
        const days = options.balance_due_days_before_event;
        return `Your remaining balance can be paid from ${dueLabel} (${days} days before your event).`;
    }
